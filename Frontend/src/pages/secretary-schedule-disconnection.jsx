...
import { Search, Calendar, AlertTriangle, Loader2 } from "lucide-react";
import { apiClient } from "../lib/api";
import { queryClient } from "../lib/query-client";
import { toast } from "sonner";

export default function SecretaryScheduleDisconnection() {
  ...
  const { data: connectionsData, isLoading, error } = useQuery({
    queryKey: ['/api/v1/water-connection/for-disconnection'],
    queryFn: () => apiClient.getConnectionsForDisconnection(),
  });
  ...
  // Get list of connection IDs that already have scheduled disconnection tasks
  const scheduledConnectionIds = new Set(
    (assignmentsData?.assignments || [])
      .filter(assignment => assignment.task?.task_type === 'disconnection')
  ...
  const createTaskMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast.success("Disconnection Task Scheduled", {
    ...
    onError: (error) => {
      toast.error("Error", {
        description: error.message || "Failed to schedule disconnection task",
      });
    },
  });

  const handleScheduleDisconnection = (connection) => {
  ...
  const handleConfirmSchedule = () => {
    ...
    // Create task data similar to incident report flow
    const taskData = {
      title: `Water Disconnection - ${selectedConnection.residentName}`,
      description: `Disconnect water service for meter ${selectedConnection.meter_no} at Purok ${selectedConnection.purok}. Resident: ${selectedConnection.residentName}`,
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <main className="flex-1 overflow-auto p-6 relative z-10">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="mb-8">
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600">Manage and schedule water service disconnections</p>
                </div>
              </div>
            </div>
            ...
            <Card className="mb-6">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-600">Pending Disconnections</p>
            ...
            <Card>
              <CardHeader>
                <CardTitle>Connections Marked for Disconnection</CardTitle>
              </CardHeader>
              <CardContent>
                {filteredConnections.length === 0 ? (
                  <div className="text-center py-8 text-gray-500">
                    <AlertTriangle className="h-12 w-12 mx-auto mb-4 text-gray-400" />
                    <p>No connections marked for disconnection</p>
                  </div>
                ) : (
                            ...
                            <TableRow key={connection.connection_id}>
                              ...
                              <TableCell>
                                <Button
                                  variant="outline"
                                  size="sm"
                                  onClick={() => handleScheduleDisconnection(connection)}
            ...
            <Dialog open={scheduleDialogOpen} onOpenChange={setScheduleDialogOpen}>
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>Schedule Disconnection Task</DialogTitle>
                  <DialogDescription>
                    This will automatically create and assign a disconnection task to available personnel.
                  </DialogDescription>
                </DialogHeader>
                ...
                {selectedConnection && (
                  <div className="py-4 space-y-3">
                    <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                      <h4 className="font-semibold text-orange-900 mb-2">Connection Details</h4>
                    ...
                    <p className="text-sm text-gray-600">
                      A disconnection task will be created and automatically assigned to available personnel based on their workload.
                    </p>
                  </div>
                )}
